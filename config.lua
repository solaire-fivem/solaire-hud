Config = {}

-- Debug
Config.Debug = true  -- Set to true to enable debug messages in the console (DISABLE IN PRODUCTION)

-- Vehicle Settings
Config.ShowMinimapAlways = false  -- Set to true to always show minimap, false to only show in vehicles
Config.SpeedMeasurement = 'mph'  -- Set to 'mph' for miles per hour or 'kph' for kilometers per hour

-- Character Sheet Settings
Config.OpenCharacterSheet = "INSERT"

--- https://fontawesome.com/icons Feel free to set the icons in the HUD to your server's style
-- Status Bar
Config.HealthBarIcon = 'fas fa-heart'  
Config.StaminaBarIcon = 'fas fa-person-running'  
Config.FoodBarIcon = 'fas fa-drumstick-bite'  
Config.WaterBarIcon = 'fas fa-bottle-water'  
Config.StressBarIcon = 'fas fa-brain'  

-- Vehicle HUD
Config.FuelIcon = 'fas fa-gas-pump'  
Config.SeatbeltIcon = 'fas fa-user-slash'  

-- Death Icon
Config.DeadIcon = 'fas fa-face-dizzy'  